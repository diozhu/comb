...
export default {
    ...
    methods: {
        init () {
            ...
            this.currentOptions = assign({}, options, {
            ...
            this.contentSource = new ContentSource(Vue, this.currentOptions);
        ...
        scrollToIndex (index) {
            if (this.waterflow) {
                for (let i = 0, len = this.scroller.items_.length; i < len; i++) {
                    if (i === index) {
                        this._scrollTo(this.scroller.items_[i].top - this.scroller.items_[i].height * this.currentOptions.column + this.$list.offsetWidth);
                    }
                }
                return;
            }
        ...
        _start (e) {
            ...
            if (this.preventDefault && !preventDefaultException(e.target, this.currentOptions.preventDefaultException)) {
                e.preventDefault();
            }
        },
        _move (e) {
            ...
            if (this.distance > this.currentOptions.distance) {
                this.distance = this.currentOptions.distance;
            }
