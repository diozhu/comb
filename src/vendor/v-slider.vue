    ...
    <div class='slider-container' :class = 'basicdata.containerClass' :style="{'height': basicdata.height}">
        ...
        >
            ...
            <template v-if="!sliderinit.loop">
                <!--预览-->
                <template v-for="(item, index) in pages" v-if="sliderinit.isPreview">
                    <div class="slider-item" :key="index">
                        <img :src="item">
                    </div>
                </template>
                <!--带title的轮播图-->
                <template v-for="(item, index) in pages" v-else>
                    <div :key="index" class="slider-item" v-if="item.title" :style="{background: 'url('+ item.url +')', 'background-size': 'cover', 'background-position': 'center', 'background-repeat': 'no-repeat'}">
                        {{item.title}}
                    </div>
                </template>
            </template>
            ...
            <template v-if="sliderinit.loop">
                ...
                <template v-for="(item, index) in pages">
                    <div :key="index" class="slider-item" :style="{background: 'url('+ item.url +')', 'background-size': 'cover', 'background-position': 'center', 'background-repeat': 'no-repeat'}">{{item.title}}</div>
                </template>
                <!-- 无缝滚动copy第一项 -->
                <template>
        ...
        <div class="slider-pagination slider-pagination-bullets" v-else>
            <template v-for="n in pagenums">
                <span :key="n" @click='slide(n-1)' class="slider-pagination-bullet" :class="n-1 === sliderinit.currentPage? 'slider-pagination-bullet-active':''"></span>
            </template>
        </div>
    </div>
</template>
