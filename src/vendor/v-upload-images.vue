<template>
    <div class="v-upload-images">
        <transition-group name="fade" tag="div" class="v-upload-images__con">
            ...
            <div v-if="typeof imgList === 'object'" v-for="(img, idx) in imgList" :key="img.id" v-show="img.src"
                 class="v-upload-images__frm" @click.stop="goViewer(idx)">
                <div class="v-upload-images__img"
                     :style="{backgroundImage: 'url(' + img.src + ')', backgroundSize: 'cover'}"></div>
                <div v-if="!img.url" class="v-upload-images__loading">
                    <v-spinner color="#FFF" type="fading-circle"></v-spinner>
                </div>
                <div v-if="!disableDel" @click.stop="delImage(idx)" class="v-upload-images__del"><i
                    class="icon icon-error"></i></div>
            </div>
...
<style rel="stylesheet/scss" lang="scss">
    ...
    // /* autoprefixer: off */ 更新eslint后报错，这里暂时关闭，再找其他解决方式~ mod by Dio Zhu. on 2018.6.7
