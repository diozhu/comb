...
//                refreshTranslate: 0,        // refresh的dom位移
                ...
                iscrollOptions: {
                    ...
                    bounce: false,
                    disableTouch: true,
                    infiniteElements: '.v-iscroll-container .row',
        ...
        watch: {
            value (val) {
                ...
                if (val.length < 1 && this.isEnabled && !this.isLoading && !this.refreshTag) { // 客户端直接清空listdata，触发刷新事件
                    this.$logger.log(`[v-iscroll].${this._uid}.watch: value`, val);
                    this.refreshList();
                }
            },
            currentValue (val) {
...
//                    this.init();
             ...
             *  之前的滚动记录需要记录在vuex上，现在改为标签属性方式，在v-iscroll-position中处理，所以不需要在此进行滚动还原
             *                  -- Author by Dio Zhu. on 2017.5.25
             */
            '$route.name' (val) {
                this.$logger.log(`v-iscroll.${this._uid}.watch: $route!!!`, val, this._inactive, this.$router.direct());
...
/*
                this.$nextTick(() => {
                    this.$logger.log(`v-iscroll.${this._uid}.watch: $route!!!`, val, this._inactive);
...
//                        this.$logger.log(`v-iscroll.${this._uid}.watch: $route!!!`, this.position[this.timestamp]);
