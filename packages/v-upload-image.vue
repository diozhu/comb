    ...
    <div v-if="mod === 'avatar'" class="v-upload-image avatar">
        ...
        <v-popup v-model="popupTag">
            <div class="avatar-demo">
                ...
                <img src="../assets/user_center_bg.png" @click.stop=""/>
                <!--微信环境显示按钮-->
    ...
    <div v-if=" mod === 'multi'" class="v-upload-image multi">
        <div v-for="(item, index) in currentValue" :key="index">
            <div :class="['frm', {loading: item.loading, fadeIn: !item.loading}]">
                ...
                <i v-if="!item.loading" class="icon icon-del" @click="handleTapDel(index)"></i>
            </div>
        </div>
        ...
        <!--</div>-->
        <!--微信环境显示按钮-->
        <button v-if="isWechat"  v-show="currentValue.length < max" @click="wxChooseImage" class="wx_upload_button wechat frm"><i class="icon icon-camera-fill"></i></button>
        <!--h5环境显示input-->
