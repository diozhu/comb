...
<script>
    import vSpinner from '../../src/vendor/v-spinner/index';
    import logger from '../../src/js/utils/logger';
    import { uploadImagesCloud } from '../../src/js/core/core';
     ...
     **注意，在core.js里。URL（516行）的地址要改为本业务真实地址，后面会优化
     */
    export default {
...
//                this.uploadedStatus = false; // 开始上传
                let max = this.max - this.imgList.length,
                    option = {
                        ...
                        'maxCount': max     // 最大可选数
                        // 'url': '//172.28.50.84:8083/authentication/uploadDown/uploadBase64BackUrl' // 图片上传的URL地址
                    };
                uploadImagesCloud(option, (e) => { // toon返回base64的回调, 用于显示图片正在上传的状态, 避免白屏等待
